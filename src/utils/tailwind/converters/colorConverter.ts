 ...
 * Color Converter
 *
 * CSS color → Tailwind 클래스 변환
 */

import type { ConversionSuggestion } from '../../../types/tailwindScanner';
import { hexToRgb, rgbToHex, colorDistance } from '../../common/colorUtils';

/**
 * Tailwind 색상 팔레트 (gray 기준)
 */
 ...
 */
function findClosestTailwindColor(hexColor: string): {
  colorName: string;
  shade: number;
  distance: number;
} | null {
  ...
  for (const [colorName, shades] of Object.entries(TAILWIND_COLORS)) {
    for (const [shadeStr, shadeHex] of Object.entries(shades)) {
...
function parseColorValue(colorValue: string): string | null {
  // 특수 색상
  const upperValue = colorValue.toLowerCase();
  if (upperValue in SPECIAL_COLORS) {
    return SPECIAL_COLORS[upperValue];
  }
  ...
  // hsl() 또는 hsla()
  const hslMatch = colorValue.match(/hsla?\((\d+),\s*(\d+)%,\s*(\d+)%/);
  if (hslMatch) {
...
export function convertColor(
  ...
  if (cssProperty.startsWith('color')) {
    prefix = 'text';
  } else if (cssProperty.startsWith('background')) {
    prefix = 'bg';
  } else if (cssProperty.startsWith('border')) {
    prefix = 'border';
  } else {
    return [{
  ...
  // 색상값 파싱
  const hexColor = parseColorValue(cssValue);
  if (!hexColor) {
    return [{
      css: `${cssProperty}: ${cssValue}`,
  ...
  if (closest) {
    suggestions.push({
      css: `${cssProperty}: ${cssValue}`,
      tailwind: `${prefix}-${closest.colorName}-${closest.shade}`,
      confidence: Math.max(0.5, 1 - closest.distance / 100),
      category: 'colors',
    isArbitrary: false,
    });
  }
 ...
 * 불투명도 변환
 */
export function convertOpacity(
  cssProperty: string,
  cssValue: string,
  baseColorClass?: string
): ConversionSuggestion[] {
  const suggestions: ConversionSuggestion[] = [];
  const opacityValue = parseFloat(cssValue);

  if (isNaN(opacityValue)) {
    return [];
  }

  // 불투명도 백분율을 Tailwind 값으로 변환
  ...
  if (baseColorClass) {
    ...
    suggestions.push({
      css: `${cssProperty}: ${cssValue}`,
      tailwind: `${baseColorClass}/${closestOpacity}`,
      confidence: 0.9,
      category: 'colors',
      isArbitrary: false,
    });
    ...
    suggestions.push({
      css: `${cssProperty}: ${cssValue}`,
      tailwind: `opacity-${closestOpacity === 100 ? '100' : closestOpacity}`,
      confidence: 0.9,
      category: 'effects',
    isArbitrary: false,
    });
