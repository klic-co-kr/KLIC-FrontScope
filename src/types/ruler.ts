 ...
 * 리사이즈 방향 (8방향)
 */
export type RulerResizeDirection = 'n' | 's' | 'e' | 'w' | 'ne' | 'nw' | 'se' | 'sw';

/**
 * 인터랙션 모드
 */
export type RulerInteractionMode = 'idle' | 'drawing' | 'moving' | 'resizing';
 ...
 * 인터랙티브 룰러 오버레이
 */
export interface InteractiveRulerOverlay {
  id: string;
  element: HTMLElement;
  label: HTMLElement;
  body: HTMLElement;
 ...
 * 룰러 인터랙션 상태
 */
export interface RulerInteractionState {
  mode: RulerInteractionMode;
  overlays: InteractiveRulerOverlay[];
  selectedOverlayId: string | null;
  activeResizeDirection: RulerResizeDirection | null;
  drawStartX: number;
  drawStartY: number;
  moveOffsetX: number;
  moveOffsetY: number;
  resizeStartBounds: OverlayBounds | null;
  resizeStartX: number;
  resizeStartY: number;
}
